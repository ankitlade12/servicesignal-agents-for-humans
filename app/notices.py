...
def render_notice(payload, public_id):
    esc = html.escape
    if not payload:
        body = f"""<span class="eyebrow">REGULAR PROGRAM · FICTIONAL DEMO</span><h1>Digital Basics</h1>
        <p class="lead">A little confidence. A world of possibilities.</p>
        <section class="notice-facts"><h2>Learn together, every Tuesday.</h2><p>{PROGRAM["schedule"]}</p>
        <p>{PROGRAM["location"]} · {PROGRAM["room"]}</p><p>Maple Community Center</p></section>
        ...
        approved = datetime.fromtimestamp(payload["approved_at"], ZoneInfo("America/Chicago")).strftime(
            "%b %d, %Y at %I:%M %p %Z"
        )
        body = f"""<span class="eyebrow">COMMUNITY NOTICE · FICTIONAL DEMO</span><h1>{heading}</h1>
        <p class="lead">{fact("program")} at Maple Community Center</p>
    ...
    <title>Digital Basics · Community notice</title><meta name="description" content="Current approved program information from Maple Community Center. Fictional demonstration.">
    <link rel="stylesheet" href="/static/styles.css"><link rel="icon" href="/static/favicon.svg" type="image/svg+xml"></head>
    <body class="public-body"><a class="skip" href="#notice">Skip to notice</a><header class="public-header"><a href="/">◈ ServiceSignal</a><span>Maple Community Center</span></header>
    ...
    <footer class="public-footer">Fictional program and addresses. This is a working demonstration, not a real service listing.</footer></body></html>"""
...
def flyer_pdf(payload, url):
    ...
    doc = SimpleDocTemplate(
        out, title="Digital Basics — community notice", author="Maple Community Center (fictional)"
    )
    f = payload["facts"]
    lines = [
