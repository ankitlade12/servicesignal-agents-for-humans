...
The Railway pilot is available at https://servicesignal-production.up.railway.app with a persistent `/app/data` volume and secure cookies. HTTPS health and readiness checks passed, and the private workspace API rejected unauthenticated access. That initial deployment used `fixture`; the later extended release below enables OpenAI. Initial owner onboarding and authenticated hosted acceptance remain outstanding. AWS Builder ID, public video upload and Devpost submission still require the entrant's account actions.
...
Release `6e7428a` passed [GitHub CI](https://github.com/ankitlade12/servicesignal-agents-for-humans/actions/runs/34807122340), including Docker build and running-container workflows. Railway deployment `7c5082c6-36ff-4f81-84d0-f39ed1c6e329` reports SUCCESS. Hosted health and readiness return 200, the private workspace rejects unauthenticated requests, the new feature bundle is served, and the provider is now OpenAI through Strands (`extended-hosted-check.json`). The hosted owner’s authenticated workflow still needs acceptance after the owner chooses their password; local pilot browser/HTTP and CI container checks are separate evidence.

The repeated-hour browser preview showed `2026-11-01T01:15:00-05:00` through `2026-11-01T01:45:00-06:00`, correctly spanning 90 elapsed minutes (`repeated-hour-review.png`).
